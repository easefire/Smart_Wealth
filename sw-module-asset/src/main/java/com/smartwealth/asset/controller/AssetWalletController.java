...
import com.smartwealth.asset.dto.PayPasswordRequest;
...
public class AssetWalletController {
    ...
    public Result<Void> setPayPassword(@Valid @RequestBody PayPasswordRequest request) {
        Long userId = UserContext.getUserId();
    ...
    public Result<String> withdraw(@Valid @RequestBody WithdrawDTO dto) {
        // 1. 从项目统一的上下文获取当前登录用户 ID
        Long userId = UserContext.getUserId();
        // 2. 调用 Service 层执行复杂的提现逻辑
